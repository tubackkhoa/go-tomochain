...
type Swap struct {
	...
	balances   map[discover.NodeID]int64 //map of balances for each peer
}
...
func New(stateStore state.Store) (swap *Swap) {
	swap = &Swap{
		...
		balances:   make(map[discover.NodeID]int64),
	}
	return
}
...
func (swap *Swap) GetPeerBalance(peer discover.NodeID) (int64, error) {
	swap.lock.RLock()
